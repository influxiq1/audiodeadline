...
app.get('/userlogin', function (req, resp) {
    ...
    var email = req.query.email;
    email = email.toLocaleString().toLowerCase();
    collection.find().toArray(function(err, items){
        if(items.length==0){
            ...
            var resemail = '';
            for(var n in items){
                var resemail = items[n].email;
                if(typeof (resemail)!='undefined' && resemail!=null)resemail = resemail.toLocaleString().toLowerCase();
                if(resemail == email){
                    resitem = items[n];
                    break;
                }
            ...
            if(resemail == email){
                ...
                if(resitem.password==hash){
                    resp.send(JSON.stringify({'status':'success','msg':resitem}));
                    return;
                }
            }else{
                resp.send(JSON.stringify({'status':'error','msg':'Email ID invalid...'}));
...
app.get('/communityuserlist',function(req,resp){
    ...
    cond = {
        $and : [{admin : 0}, {$or: [{musicians : 1}, {dancer : 1}, {model : 1}, {fan : 1},{signupaffiliate: 1}]},{ fullname: { $nin: [ null, "" ] } }]
    };

    if(req.query.username != ''){
        cond = {
            $and : [{admin : 0}, {$or: [{musicians : 1}, {dancer : 1}, {model : 1}, {fan : 1},{signupaffiliate: 1}]}, {parent:req.query.username},{ fullname: { $nin: [ null, "" ] } }]
        };
    }
...
app.get('/addpromocode',function(req,resp){
    ...
    promocode = promocode.toLocaleString().toUpperCase();
...
app.get('/chkpromocode1',function(req,resp){
    var collection= db.collection('promocodeview');
    var promocode = req.query.promocode;
    promocode = promocode.toLocaleString().toUpperCase();
    cond = {
...
function getfixedval(val){
    console.log(val);
    if(typeof (val)!='undefined')return parseFloat(val).toFixed(2);
    else return '';
}
...
app.get('/ticketsale',function(req,resp){
    ...
    resp.send(JSON.stringify({'status':'error','msg':'Database error occurred.',req:req.query}));
    return;
    }
    var email = req.query.email;
    if(typeof (req.query.email) != 'undefined' && typeof (req.query.email) != ''){
    ...
    db.collection('user').find({email:req.query.email}).toArray(function(err1,items1){
        if(err1){
            ...
            if(items1.length > 0){
                userid = items1[0]._id;
                //resp.send(JSON.stringify({'1status':'error','msg':'Database error occurred.',req:req.query}));
                //return;

                db.collection('user').update({username: items1[0].username}, {$set: {cardnumber: req.query.cardnumber,vaultid: req.query.token}});
                if(typeof(req.query.cardnumber)!='undefined' && req.query.cardnumber.length>1) {
                    db.collection('usertoken').insert([{
                ...
                //resp.send(JSON.stringify({'status3':'success','msg':'Order Succesful.'}));
                //return;

                var promocode = req.query.promocode;
                if(typeof(req.query.promocode)!='undefined' && req.query.promocode.length>1)promocode = promocode.toLocaleString().toUpperCase();
                var subamount = req.query.subamount;
                var subamount2 = req.query.subamount;
                var amount = req.query.amount;
                var disamount = req.query.disamount;
                subamount2 = parseFloat(req.query.subamount*req.query.qty);
                //if(typeof (req.query.subamount)!='undefined' )subamount = subamount.toFixed(2);
                console.log(subamount2);
                //if(typeof (req.query.subamount)!='undefined') subamount2 = subamount.toFixed(2);
                amount = parseFloat(amount);
                amount = amount.toFixed(2);
                disamount = parseFloat(disamount*req.query.qty);
                disamount = disamount.toFixed(2);
                var productid = req.query.productid;
                productid = new mongodb.ObjectID(productid);

                //resp.send(JSON.stringify({'status1':'success','msg':'Order Succesful.'}));
                //return;
                var promohtml = '';
                if(promocode !=''){
                    ...
                    if (err2) {
                        ...
                        if(req.query.media == ''){
                            ...
                            /*db.collection('sponsormailbody').find({sponsor: req.query.media}).toArray(function(err, items) {
                             ...
                             resitem = items[0];
                             if(typeof(resitem.mailbody) == 'undefined' || resitem.mailbody == ''){
                             resp.send(JSON.stringify({'status':'success','msg':'Order Succesful.'}));
                             }else{
                             var mail = {
                             from: "Admin <itplcc40@gmail.com>",
                ...
                //resp.send(JSON.stringify({'status09':'error','msg':'Database error occurred.',req:req.query}));
                //return;
                db.collection('user').insert([{
                    ...
                    commission: 0,
                    cardnumber: req.query.cardnumber,
                    vaultid: req.query.token
                }], function (err2, result2){
                    if(err2){
                        resp.send(JSON.stringify({'status':'error','msg':'Database error occurred.'}));
                    }else{
                        //resp.send(JSON.stringify({'status93':'error','msg':'Database error occurred.',req:req.query}));
                        //return;

                        userid = result2.insertedIds[0];
                        if(req.query.cardnumber.length>1) {
                            db.collection('usertoken').insert([{
                                userid: new mongodb.ObjectID(userid),
                                added_time: moment().unix(),
                                cardnumber: req.query.cardnumber,
                        ...
                        subamount2 = parseFloat(subamount*req.query.qty);
                        ...
                        db.collection('order').insert([{
                            ...
                            if (err2) {
                                ...
                                var mail = {
                                    ...
                                    to: req.query.email,
                                    subject: 'Your Ticket Order Was Successful!',
                                    html: '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><title>Retail customer connect</title></head><body><div style="width:640px; margin:0 auto; background:#000; padding:20px;"><div style="width:620px;"><div style="width:100%; background:#1d1f20; padding:10px; border-bottom: solid 1px #1d1f20; box-shadow:0 0 10px #888;"><table width="100%" border="0" cellspacing="0" cellpadding="0" style="background:#afc5d8; padding:20px; border-bottom:solid 2px #ffc823;"><tr><td align="left" valign="middle"><img src="https://audiodeadline.com/assets/images/logo_mail.png"  alt="#" width="250"/></td></tr></table><table width="100%" border="0" cellspacing="0" cellpadding="0"><tbody><tr><td align="center"><div style="margin:0 auto;font-family:Arial,Helvetica,sans-serif;background:#fff"><table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding:40px 0;background: #415f79;font-family:Arial,Helvetica,sans-serif"><tbody><tr><td align="left" valign="top"><div style="border-left:solid 6px #ffc823;padding:6px 6px 6px 10px"><h2 style="color:#fff;font-size:18px;font-weight:normal;margin:0;padding:0px;display:inline-block">Invoice to:</h2><br><h3 style="font-weight:normal;margin:5px 0 8px 0;padding:0px;font-size:22px;color:#efc956;display:inline-block">'+ req.query.firstname +' '+ req.query.lastname +'</h3><br><h4 style="font-weight:normal;margin:0;padding:0;font-size:14px;line-height:20px;color:#fff;display:inline-block; text-decoration:none;">'+ req.query.address +', '+ req.query.city +' '+ req.query.zip +', US<br /><a href="mailto:'+ req.query.email +'" target="_blank" style="color:#fff; display:inline-block; text-decoration:none;">'+ req.query.email +'</a></h4></div></td><td align="right" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0"><tbody><tr><td align="right" valign="middle" style="padding:2px 8px 0 0;color:#fff;font-size:14px">Invoice date: '+ moment().format("Do, MMM YYYY") +'</td></tr><tr><td align="right" valign="middle" style="padding:15px 8px 0 0;color:#fff;font-size:20px">Total amount:$'+ amount +' </td></tr></tbody></table></td></tr></tbody></table><table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial,Helvetica,sans-serif"><tbody><tr><th width="2%" align="left" valign="middle" style="background:#5581a7">&nbsp;</th><th width="47%" align="left" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal">Item Description</th><th width="5%" align="left" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal"><img src="https://audiodeadline.com/assets/images/rightarrow_mail.png" alt="#"></th><th width="9%" align="center" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal"> Price</th><th width="5%" align="left" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal"><img src="https://audiodeadline.com/assets/images/rightarrow_mail.png" alt="#"></th><th width="8%" align="center" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal">Qty. </th><th width="5%" align="left" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal"><img src="https://audiodeadline.com/assets/images/rightarrow_mail.png" alt="#"></th><th width="16%" align="center" valign="middle" style="background:#5581a7;padding:8px 10px;font-size:16px;font-weight:bold!important;color:#fff;font-weight:normal">Total </th><th width="2%" align="left" valign="middle" style="background:#5581a7">&nbsp;</th></tr><tr><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b;">&nbsp;</td><td align="left" valign="middle" style="padding:8px 10px;font-size:16px;color:#111;font-weight:normal;border-bottom:solid 2px #9e9b9b">Ticket</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b;">&nbsp;</td><td align="center" valign="middle" style="padding:8px 10px;font-size:16px;color:#111;font-weight:normal;border-bottom:solid 2px #9e9b9b">$'+ subamount +'</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b;">&nbsp;</td><td align="center" valign="middle" style="padding:8px 10px;font-size:16px;color:#111;font-weight:normal;border-bottom:solid 2px #9e9b9b">1</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b;">&nbsp;</td><td align="center" valign="middle" style="padding:8px 10px;font-size:16px;color:#111;font-weight:normal;border-bottom:solid 2px #9e9b9b">$'+ getfixedval(subamount2) +'</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b;">&nbsp;</td></tr></tbody></table><table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding:20px 0px;margin:0;font-family:Arial,Helvetica,sans-serif;font-size:20px;color:#333; background: #425f78;"><tbody><tr><td width="100%"><table width="100%" border="0" cellspacing="0" cellpadding="0"><tbody><tr><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b">&nbsp;</td><td width="62%" align="right" valign="middle" style="color:#fff;padding:5px;border-bottom:solid 2px #9e9b9b;font-family:Arial,Helvetica,sans-serif;font-size:18px!important">Subtotal</td><td align="right" valign="middle" style="color:#fff;padding:5px;border-bottom:solid 2px #9e9b9b;font-family:Arial,Helvetica,sans-serif;font-size:18px!important">$'+ getfixedval(subamount) +'</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b">&nbsp;</td></tr><tr><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b">&nbsp;</td><td width="62%" align="right" valign="middle" style="color:#fff;padding:5px;border-bottom:solid 2px #9e9b9b;font-family:Arial,Helvetica,sans-serif;font-size:18px!important">Tax</td><td align="right" valign="middle" style="color:#fff;padding:5px;border-bottom:solid 2px #9e9b9b;font-family:Arial,Helvetica,sans-serif;font-size:18px!important">$0.00</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b">&nbsp;</td></tr><tr><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b">&nbsp;</td><td width="62%" align="right" valign="middle" style="color:#fff;padding:5px;border-bottom:solid 2px #9e9b9b;font-family:Arial,Helvetica,sans-serif;font-size:18px!important">Shipping</td><td align="right" valign="middle" style="color:#fff;padding:5px;border-bottom:solid 2px #9e9b9b;font-family:Arial,Helvetica,sans-serif;font-size:18px!important">$0.00</td><td align="left" valign="middle" style="border-bottom:solid 2px #9e9b9b">&nbsp;</td></tr>'+ promohtml +'<tr><td align="left" valign="middle" style="background:#8c8c8c">&nbsp;</td><td width="62%" align="right" valign="middle" style="padding:5px;color:#fff;background:#8c8c8c;font-size:22px!important">Grand Total</td><td align="right" valign="middle" style="padding:5px;color:#fff;background:#8c8c8c">$'+ amount +'</td><td align="left" valign="middle" style="background:#8c8c8c">&nbsp;</td></tr></tbody></table></td></tr></tbody></table><div style="width:auto;padding:30px;text-align:center;background:#f9bc07;color:#000; font-weight: bold; text-align:center;margin-top:20px">Thank you For Your Purchase Order</div></div></td></tr></tbody></table></div></div></div></body></html>'
                                }
...
app.get('/myorder',function(req,resp){
    var collection= db.collection('orderview');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/mytoken',function(req,resp){
    var collection= db.collection('usertoken');
    var o_id = new mongodb.ObjectID(req.query._id);

    collection.find({userid:o_id,$where:'this.token.length>3'}).toArray(function(err, items) {
        if (!items) {
            resp.send(JSON.stringify({'status':err,'item':[]}));
        } else {
            resp.send(JSON.stringify({'status':'success','item':items,c:items.length}));
        }
    });
});
...
app.get('/competitionlist',function(req,resp){
    var collection= db.collection('competitionview');
    var cond = {};
    cond = { competitionname: { $nin: [ null, "" ] } };

    if(req.query.type == 'active'){
        cond = {
            $and : [
                { competitionname: { $nin: [ null, "" ] } },
                {status: 1 }
            ]
        };
    }
...
app.get('/addcompetition',function(req,resp){
    var collection= db.collection('competition');

    collection.insert([{
        competitionname: req.query.competitionname,
        description: req.query.description,
...
app.get('/cngstatuscompetition',function(req,resp){
    var collection= db.collection('competition');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/deletecompetition',function(req,resp){
    var collection= db.collection('competition');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/competitiondetails',function(req,resp){
    var resitem = {};
    var collection = db.collection('competition');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/updatecompetition',function(req,resp){
    var collection= db.collection('competition');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/updatecompetitionsettings',function(req,resp){
    var collection= db.collection('competition');
    var o_id = new mongodb.ObjectID(req.query._id);

    collection.update({_id: o_id}, {$set: {
        backstoryrequired: parseInt(req.query.backstoryrequired),
        photorequired: parseInt(req.query.photorequired),
        videorequired: parseInt(req.query.videorequired),
        musicrequired: parseInt(req.query.musicrequired),
        noofphoto: parseInt(req.query.noofphoto),
        noofvideo: parseInt(req.query.noofvideo),
        noofmusic: parseInt(req.query.noofmusic),
    }},function(err, results) {
        resp.send(JSON.stringify({'status':'success'}));
    });
});
...
app.get('/medialist',function(req,resp){
    ...
    if(req.query.isadmin != '1'){
        cond = {
            $and : [{uploadedby :new mongodb.ObjectID(req.query.userid)},{ name: { $nin: [ null, "" ] } }]
        };
    }

    collection.find(cond).sort( { sortindex: 1 } ).toArray(function(err, items){
        if(err || items.length==0){
            resp.send(JSON.stringify({'res':[]}));
        }else{
            resp.send(JSON.stringify({'res':items}));
...
app.get('/addmedia',function(req,resp){
    var name = '';
    if(req.query.label !=''){
        name = req.query.label;
        name = name.trim();
        name = name.toLowerCase();
        var namearr = name.split(" ");
        namearr = namearr.filter(function (el) {
            if(el != '')
                return el;
        });
        name = namearr.join('-');
    ...
    if(req.query.type == '2'){
        name = req.query.sponsor+'-'+name;
    }
    if(req.query.type == '3'){
        name = 'audiodeadline-'+name;
    }
    if(req.query.type == '4'){
        name = 'artistxp-'+name;
    }
    if(req.query.type == '5'){
        name = 'audiodeadline-instagram-'+name;
    }
    if(req.query.type == '6'){
        name = 'artistxp-instagram-'+name;
    }
    var data = {
        label: req.query.label,
        name: name,
        image: req.query.image,
        origimage: req.query.origimage,
        image2: req.query.image2,
        origimage2: req.query.origimage2,
        image3: req.query.image3,
        origimage3: req.query.origimage3,
        type: parseInt(req.query.type),
        sponsor: req.query.sponsor,
        added_time: moment().unix(),
        status: status, //1=>Active; 2=>Inactive; 3=>Pending
        noofclick: 0,
        sortindex: 0,
        uploadedby: new mongodb.ObjectID(req.query.userid),
    };


    var collection= db.collection('media');

    collection.find({name:data.name}).toArray(function(err22, items22) {
        if(items22.length>0){
            resp.send(JSON.stringify({'status':'error','msg':'This name is already exist..!'}));
            return;
        }else{
...
app.get('/cngstatusmedia',function(req,resp){
    var collection= db.collection('media');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/deletemedia',function(req,resp){
    var collection= db.collection('media');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/mediadetails',function(req,resp){
    var resitem = {};
    var collection = db.collection('media');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/updatemedia',function(req,resp){
    var collection= db.collection('media');
    var o_id = new mongodb.ObjectID(req.query._id);
...
app.get('/updatemediainline',function(req,resp){
    var collection= db.collection('media');
    var o_id = new mongodb.ObjectID(req.query._id);

    var data = {};

    if(req.query.fieldname == 'sortindex'){
        data = {sortindex: parseInt(req.query.filedvalue)};
        collection.update({_id: o_id}, {$set: data},function(err, results) {
            resp.send(JSON.stringify({'status':'success'}));
        });
    }else if(req.query.fieldname == 'status'){
        data = {status: parseInt(req.query.filedvalue)};
        collection.update({_id: o_id}, {$set: data},function(err, results) {
            resp.send(JSON.stringify({'status':'success'}));
        });
    }else if(req.query.fieldname == 'label'){
        var name = '';
        name = req.query.filedvalue;
        name = name.trim();
        name = name.toLowerCase();
        var namearr = name.split(" ");
        namearr = namearr.filter(function (el) {
        ...
        collection.find({_id:o_id}).toArray(function(err, items) {
            if(err){
                ...
                data = {name: name, label:req.query.filedvalue};
                collection.find({name:data.name}).toArray(function(err22, items22) {
                    if(items22.length>0){
                        resp.send(JSON.stringify({'status':'error','msg':'*This name is already exist..!'}));
                    }else{
...
app.get('/medialistbytype',function(req,resp){
    ...
    var cond = {};
    cond = {
        $and : [{type : parseInt(req.query.type)},{status : 1},{ name: { $nin: [ null, "" ] } }]
        //$and : [{type : parseInt(req.query.type)},{ name: { $nin: [ null, "" ] } }]
    };
...
app.get('/getsponsormedianew',function(req,resp){
    var collection= db.collection('mediaview');
    var cond = { name: req.query.media };
...
app.get('/competitionsignup',function(req,resp) {
    var genre = '';
    if(req.query.genre != ''){
    ...
    if(req.query.competitionid != ''){
        competitionid = new mongodb.ObjectID(req.query.competitionid);
    }
    var data = {
        name: req.query.name,
        email: req.query.email,
        genre: genre,
        bio: req.query.bio,
        whydoyouwant: req.query.whydoyouwant,
        userid: userid,
        competitionid: competitionid,
        backstorytype: req.query.backstorytype,
        backstoryval: req.query.backstoryval,
        images: req.query.images,
        videos: req.query.videos,
        musics: req.query.musics,
        status: 0,
        added_time: moment().unix(),
    };

    db.collection('competitionuser').insert([data], function (err, result) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','msg':'Database error occured. Try again!'}));
        } else {
            resp.send(JSON.stringify({'status': 'success'}));
...
app.get('/getcompetitionsignuplist',function(req,resp){
    var collection= db.collection('competitionuserview');
    var cond = { };
...
app.get('/cngstatuscompuser',function(req,resp){
    var collection= db.collection('competitionuser');
    var o_id = new mongodb.ObjectID(req.query._id);

    collection.update({_id: o_id}, {$set: {status: parseInt(req.query.filedvalue)}},function(err, results) {
        resp.send(JSON.stringify({'status':'success'}));
    });
});
...
app.get('/getchatforthischatwindow',function(req,resp){
    var collection= db.collection('chatdetails');
    var collection1= db.collection('chatdetails_view');
    var o_id = new mongodb.ObjectID(req.query.id);
    var o_id1 = new mongodb.ObjectID(req.query.from);
    collection.update({chatto: o_id,is_read:0,chatfrom:new mongodb.ObjectID(req.query.from)}, {$set: {is_read:1}},function(err, results) {

        //resp.send(JSON.stringify({'status':'success',id:o_id,o:0}));
        collection1.find({$or: [{$and : [{'chatto':o_id},{'chatfrom':o_id1}]},{$and : [{'chatto':o_id1},{'chatfrom':o_id}]}] }).limit(20).toArray(function(err, itemsval) {
            if (err) {
                console.log(err);
                resp.send(JSON.stringify({'status':'error','item':[]}));
            } else {
                resp.send(JSON.stringify({'status':'success','items':[],newchats:itemsval}));
            }
        });
    });
});

app.get('/updategeneralchatstatus',function(req,resp){
    var collection= db.collection('user');
    collection.update({ lastactivetime: { $lt: parseInt(moment().unix()-45) } ,is_chatactive:1 }, {$set: {is_chatactive:0}},function(err, results) {
        //resp.send(JSON.stringify({'status':'success','res':results}));

        collection.find({ lastactivetime: { $lt: parseInt(moment().unix()-45) },is_chatactive:1}).limit(8888888).toArray(function(err, itemsval) {
            if (err) {
                console.log(err);
                resp.send(JSON.stringify({'status':'error','item':[]}));
            } else {
                resp.send(JSON.stringify({'status':'success','items':itemsval.length,newchats:itemsval}));
            }
        });
    });
});
app.get('/updatechatstatus',function(req,resp){
    setTimeout(function () {
        var collection= db.collection('user');
        //console.log('req.query.id');
        ///console.log(req.query);
        //console.log(req.query.id);
        if(typeof(req.query.id)=='undefined' || req.query.id.length<5){
            resp.send(JSON.stringify({'status':'error','item':[]}));
            return;
        }
        var o_id = new mongodb.ObjectID(req.query.id);

        collection.update({ lastactivetime: { $lt: parseInt(moment().unix()-45) } }, {$set: {is_chatactive: 0}},function(err, results) {
            //resp.send(JSON.stringify({'status':'success'}));
        });

        collection.update({_id: o_id}, {$set: {is_chatactive: 1,lastactivetime:moment().unix()}},function(err, results) {
            //resp.send(JSON.stringify({'status':'success'}));
            ...
            collection1.find({is_read:0,chatto:o_id}).toArray(function(err, items) {
                if (err) {
                    console.log(err);
                    resp.send(JSON.stringify({'status':'error','item':[]}));
                } else {
                    ...
                    newchat=items;
                    collection1.find(cond).toArray(function(err, itemsval) {
                        if (err) {
                            console.log(err);
                            resp.send(JSON.stringify({'status':'error','item':[]}));
                        } else {
                            resp.send(JSON.stringify({'status':'success','items':newchat,newchats:itemsval}));
                        }
                    });
...
app.get('/postchat',function(req,resp){
    setTimeout(function () {
        var collection= db.collection('chatdetails');
        var collection1= db.collection('chatdetails_view');
        var o_id = new mongodb.ObjectID(req.query.user_id);
        var o_id1 = new mongodb.ObjectID(req.query.sendto);
        ...
        db.collection('chatdetails').insert([data], function (err, result) {
            if (err) {
                resp.send(JSON.stringify({'status':'error','msg':'Database error occured. Try again!'}));
            } else {
                ...
                collection1.find({$or: [{$and : [{'chatto':o_id},{'chatfrom':o_id1}]},{$and : [{'chatto':o_id1},{'chatfrom':o_id}]}] }).limit(20).toArray(function(err, itemsval) {
                    if (err) {
                        ...
                        resp.send(JSON.stringify({'status':'success','items':itemsval}));
                    }
                });
                collection.update({chatfrom: o_id1,chatto:o_id}, {$set: {is_read:1}},function(err, results) {
                    //resp.send(JSON.stringify({'status':'success'}));
                });
            }
        });
...
app.get('/getchatforauser',function(req,resp){
    setTimeout(function () {
        var collection= db.collection('chatdetails');
        var collection1= db.collection('chatdetails_view');
        var o_id = new mongodb.ObjectID(req.query.user_id);
        ...
        collection1.find({$or: [{$and : [{'chatto':o_id},{'chatfrom':o_id1}]},{$and : [{'chatto':o_id1},{'chatfrom':o_id}]}] }).limit(20).skip(parseInt(req.query.skip)).toArray(function(err, itemsval) {
            if (err) {
                console.log(err);
                resp.send(JSON.stringify({'status':'error','item':[]}));
            } else {
...
app.get('/getuserdetailsbyfriendtype',function (req,resp) {
    var collection= db.collection('friendview');
