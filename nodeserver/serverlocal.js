    ...
    collection.find().toArray(function(err, items){
        if(items.length==0){
            ...
            for(var n in items){
                ...
                resemail = resemail.toLocaleString().toLowerCase();
                if(resemail == email){
                    resitem = items[n];
                    break;
                }
...
app.get('/ticketsale',function(req,resp){
    ...
    var token = '';
    var email = req.query.email;
    if(typeof (req.query.token) != 'undefined' && typeof (req.query.token) != ''){
    ...
    db.collection('user').find({email:req.query.email}).toArray(function(err1,items1){
        if(err1){
            ...
            if(items1.length > 0){
                ...
                db.collection('user').update({username: items1[0].username}, {$set: {cardnumber: req.query.cardnumber,vaultid: token}});
