...
app.get('/updateNotification',function(req,resp){

    var collection = db.collection('notification');
    var collection1 = db.collection('userview_trending');
    var collection2 = db.collection('videolikes');
    var collection3 = db.collection('comment');
    ...
    collection1.find().toArray(function(err, items){
        if(err){
            ...
            var itemsval = items;
            //resp.send(JSON.stringify({'item':itemsval}));
            for (var i in itemsval){

                if(typeof (itemsval[i].videodetails_title)!='undefined' ){
                    var cond='videopost';
                }
                else if(typeof (itemsval[i].musicdetails_title_music)!='undefined' ){
                    var cond='musicpost';
                }
                else if(typeof (itemsval[i].picturedetails_title_pic)!='undefined'){
                    var cond='picturepost';
                }

                for(var k in itemsval[i].post_id){

                    for(var m in itemsval[i].post_time) {

                        collection.insert([{
                            actionby: new mongodb.ObjectID(itemsval[i]._id),
                            post_id: new mongodb.ObjectID(itemsval[i].post_id[k]),
                            actiontype: cond,
                            post_added_time: itemsval[i].post_time[m],
    ...
    collection2.find().toArray(function (err,items) {
        ...
        if(err){
            ...
            var itemsLike = items;
            //resp.send(JSON.stringify({'items':itemsLike}));
            for( var j in itemsLike){
                collection.insert([{
                    actionby: new mongodb.ObjectID(itemsLike[j].user_id),
                    post_id: new mongodb.ObjectID(itemsLike[j].videoid),
                    actiontype: 'like',
                    post_added_time: itemsLike[j].added_time,
                    last_updated: moment().unix()
                }], function (err, result) {
                    if (err) {
                        //resp.send(JSON.stringify({'status':'failed'}));
    ...
    collection3.find().toArray(function (err,items) {
        ...
        if(err){
            ...
            var itemsComments = items;
            //resp.send(JSON.stringify({'items':itemsLike}));
            for( var p in itemsComments){
                collection.insert([{
                    actionby: new mongodb.ObjectID(itemsComments[p].user_id),
                    post_id: new mongodb.ObjectID(itemsComments[p].post_id),
                    actiontype: 'comment',
                    post_added_time: itemsComments[p].added_time,
                    last_updated: moment().unix()
                }], function (err, result) {
                    if (err) {
                        //resp.send(JSON.stringify({'status':'failed'}));
...
app.get('/gethistory', function (req, resp) {
    var collection = db.collection('notification_view');
    var o_id = new mongodb.ObjectID(req.query._id);

    collection.find({actionby:o_id}).limit(parseInt(50)).skip(0).toArray(function(err, items){
...
app.get('/userlistforblocking',function(req,resp){
    //resp.send(JSON.stringify({'res':[]}));
    var collection= db.collection('userview');
    var cond = {};
    /*cond = {
    ...
    collection.find({status:1,admin:0}, {_id:1,  fullname:1, firstname:1, lastname:1, images:1}).toArray(function(err, items){
        if(err || items.length==0){
            resp.send(JSON.stringify({'res':[]}));
        }else{
            resp.send(JSON.stringify({'res':items}));
...
app.get('/blockeduserlist',function(req,resp){
    //resp.send(JSON.stringify({'res':[]}));
    var collection= db.collection('userblocklistview');
    collection.find({blocked_by_id: new mongodb.ObjectID(req.query.blocked_by_id)}).toArray(function(err, items){
        if(err || items.length==0){
            resp.send(JSON.stringify({'res':[]}));
        }else{
            resp.send(JSON.stringify({'res':items}));
...
app.get('/unblockuser',function(req,resp){
    //resp.send(JSON.stringify({'res':[]}));
    var collection= db.collection('userblocklist');
    var cond = {};
    cond = {
     $and : [
     {blocked_by_id: new mongodb.ObjectID(req.query.blocked_by_id)},
     {blocked_id: new mongodb.ObjectID(req.query.blocked_id)},
     ]
     };
    collection.remove(cond, true);
    resp.send(JSON.stringify({'status':'success'}));



});
app.get('/getblocklistbyuserid',function(req,resp){
    //resp.send(JSON.stringify({'res':[]}));
    if(req.query.user_id==''){
        resp.send(JSON.stringify({'status':'success','res':[]}));
        return;
    }
    var collection= db.collection('userblocklist');
    var cond = {};
    cond = {
     $or : [
     {blocked_by_id: new mongodb.ObjectID(req.query.user_id)},
     {blocked_id: new mongodb.ObjectID(req.query.user_id)},
     ]
     };
    collection.find(cond,{blocked_id:1,blocked_by_id:1}).toArray(function(err, items){
        if(err || items.length==0){
            resp.send(JSON.stringify({'status':'failed','res':[]}));
        }else{
            resp.send(JSON.stringify({'status':'success','res':items}));
        }
    });
...
app.get('/getDetailsByUserID',function(req,resp){

    var collection = db.collection('user');
    collection.find({_id:new mongodb.ObjectID(req.query._id)}).toArray(function(err, items) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','id':0}));
        } else {
...
app.get('/trendingVideoListwithlimit',function(req,resp){
    ...
    if(typeof (req.query.limit)=='undefined') {
        resp.send(JSON.stringify({'status':'success','item':[],'id_':req.query.blockedidarray}));
        return;
    }
    var collection = db.collection('uservideolistView');


     //formycomments and likes
    if(typeof(req.query.flagval)!='undefined' && typeof(req.query.flagval.type)!='undefined' && req.query.flagval.type=='mylikes'){
        for(var i in req.query.flagval.arr){
            req.query.flagval.arr[i]=new mongodb.ObjectID(req.query.flagval.arr[i]);
        }
        var collection1 = db.collection('videolikes');
        collection1.find({user_id:{$in:req.query.flagval.arr}}).limit(parseInt(500)).skip(parseInt(0)).toArray(function(err, litems) {
            if (err) {
                resp.send(JSON.stringify({'status':'error','id_':req.query.blockedidarray}));
            } else {
                var temlikarr=[];
                for(var x  in litems){
                    temlikarr.push(litems[x].videoid)
                }
                console.log('litems');
                console.log(litems);
                console.log(litems.length);
                console.log(temlikarr);
                console.log(temlikarr.length);
                // resitem = items[0];
                var collection2 = db.collection('comment');
                collection2.find({user_id:{$in:req.query.flagval.arr}}).limit(parseInt(500)).skip(parseInt(0)).toArray(function(err, citems) {
                    if (err) {
                        resp.send(JSON.stringify({'status':'error','id_':req.query.blockedidarray}));
                    } else {
                        for(var x  in citems){
                            temlikarr.push(citems[x].post_id);
                        }
                        // resitem = items[0];
                        cond={isartistfeed:'1',_id:{$in: temlikarr},userdata_fan:parseInt(req.query.userdata_fan)  };
                        collection.find(cond).limit(parseInt(req.query.limit)).skip(parseInt(req.query.skip)).toArray(function(err, items) {
                            if (err) {
                                resp.send(JSON.stringify({'status':'error','id_':req.query.blockedidarray}));
                            } else {
                                // resitem = items[0];
                                console.log(items.length);
                                resp.send(JSON.stringify({'status':'success','item':items,'id_':req.query.blockedidarray}));
                            }
                        });
                    }
                });
    ...
    //my commentblock ends here.

    if(typeof(req.query.blockedidarray)=='undefined') req.query.blockedidarray=[];
    else {
        for(var c in req.query.blockedidarray){

            req.query.blockedidarray[c]=new mongodb.ObjectID(req.query.blockedidarray[c]);
        }
    }

    for(var i in req.query.favoritesarray){

        req.query.favoritesarray[i]=new mongodb.ObjectID(req.query.favoritesarray[i]);
    }
    for(var j in req.query.friendsarray){

        req.query.friendsarray[j]=new mongodb.ObjectID(req.query.friendsarray[j]);
    }

    if(typeof(req.query.flagval)!='undefined' && typeof(req.query.flagval.type)!='undefined' && req.query.flagval.type=='myposts'){
        for(var i in req.query.flagval.arr){
    ...
    var cond={};
    if(typeof (req.query.flagval)=='undefined' || req.query.flagval==0) {
        /*cond = {user_id:{$nin: req.query.blockedidarray}};*/
        cond={privacy:"public",isartistfeed:'1',user_id:{$nin: req.query.blockedidarray} };
    }
    if(req.query.flagval=='favouriteflag') {
        /*cond = {user_id:{$nin: req.query.blockedidarray}};*/
        cond={privacy:"public",isartistfeed:'1',user_id:{$in: req.query.favoritesarray} };
    }

    if(req.query.flagval=='friendflag') {
        /*cond = {user_id:{$nin: req.query.blockedidarray}};*/
        cond={privacy:"public",isartistfeed:'1',user_id:{$in: req.query.friendsarray} };
    }

    if(typeof(req.query.flagval)!='undefined' && typeof(req.query.flagval.type)!='undefined' && req.query.flagval.type=='myposts') {
        /*cond = {user_id:{$nin: req.query.blockedidarray}};*/
        cond={privacy:"public",isartistfeed:'1',user_id:{$in: req.query.flagval.arr} };
    }
    /*if(req.query.flagval=='favouriteflag' || req.query.flagval=='friendflag' && 8==8) {
        resp.send(JSON.stringify({'status': 'success', 'id_': req.query, iu: cond}));
        return;
    }*/
...
app.get('/getPlaylistByUserid',function(req,resp){
...
app.get('/adduploadvideos',function(req,resp){
    var collection= db.collection('video');

    var x;

    var addvideoarray = req.query.addvideoarray;
    for(x in addvideoarray){

        addvideoarray[x].added_time = moment().unix();
        addvideoarray[x].user_id =  new mongodb.ObjectID(req.query.user_id)

    }
    collection.insert(addvideoarray, function (err, result) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','msg':'Database error occured. Try again!'}));
        } else {

            var results = result.ops;
            //resp.send(JSON.stringify(results));
            for( y in results){

                addhistorydata(results[y]._id,results[y].user_id,'videopost',moment().unix());
            }
            resp.send(JSON.stringify({'status': 'success','item':result}));
...
app.get('/adduploadmusics',function(req,resp){
    var collection= db.collection('music');

    var x;

    var addmusicarray = req.query.addmusicarray;
    for(x in addmusicarray){

        addmusicarray[x].added_time = moment().unix();
        addmusicarray[x].user_id =  new mongodb.ObjectID(req.query.user_id)

    }
    collection.insert(addmusicarray, function (err, result) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','msg':'Database error occured. Try again!'}));
        } else {
            ...
            for( y in results){
                ...
                addhistorydata(results[y]._id,results[y].user_id,'musicpost',moment().unix());
            }
            resp.send(JSON.stringify({'status': 'success','item':result}));
        }
    });
...
app.get('/adduploadpics',function(req,resp){
    var collection= db.collection('pictures');

    var x;

    var addpicturearray = req.query.addpicturearray;
    for(x in addpicturearray){

        addpicturearray[x].added_time = moment().unix();
        addpicturearray[x].user_id =  new mongodb.ObjectID(req.query.user_id)

    }
    collection.insert(addpicturearray, function (err, result) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','msg':'Database error occured. Try again!'}));
        } else {
            ...
            for( y in results){
                ...
                addhistorydata(results[y]._id,results[y].user_id,'picturepost',moment().unix());
            }
            resp.send(JSON.stringify({'status': 'success','item':result}));
        }
    });
...
app.get('/updatevideos',function(req,resp){
    var collection= db.collection('video');
    var x;
    var data;
    ...
    for(x in req.query.data){
        setTimeout(function () {
            ...
            data={privacy: req.query.data[x].privacy,title: req.query.data[x].title};
            collection.update({_id: new mongodb.ObjectID(req.query.data[x]._id)}, {$set: data},function(err, results) {
                if(err){
                    resp.send(JSON.stringify({'status':'error'}));
                    return;
                }else{
...
app.get('/addplaylist',function(req,resp){
    var collection= db.collection('playlist');
...
//add data in history collection
function addhistorydata(post_id,user_id,actiontype,post_added_time){
    var collection = db.collection('notification');
    collection.insert([{

        post_id: new mongodb.ObjectID(post_id),
        actionby: new mongodb.ObjectID(user_id),
        last_updated: moment().unix(),
        actiontype:actiontype,
        post_added_time: post_added_time,
        ...
        if (err) {
            ...
            collection1.find({post_id:new mongodb.ObjectID(req.query.post_id)}).toArray(function(err, items) {
                if (err) {
                    ...
                    // resitem = items[0];
                    resp.send(JSON.stringify({'status':'success','item':items}));
                    addhistorydata(req.query.post_id,req.query.user_id,'comment',moment().unix());
                    return;
                }
            });
            // /resp.send(JSON.stringify({'status': 'success','item':items}));
...
app.get('/addvideoviewsandgetupdate',function(req,resp){
    //var collection= db.collection('videoviews');
    var added_time= moment().unix();
            ...
            if(items.length==0){
                ...
                collection.insert([{
                    ...
                    if (err) {
                        ...
                        //resp.send(JSON.stringify({'status': 'success'}));
                        collection.find({videoid:new mongodb.ObjectID(req.query.videoid)}).toArray(function(err, ival) {
                            resp.send(JSON.stringify({'status': 'success',countval:ival.length}));
                        });
                    }
                });
...
app.get('/addmusicviewsandgetupdate',function(req,resp){
    //var collection= db.collection('videoviews');
    var added_time= moment().unix();

    var collection = db.collection('musicviews');
    collection.deleteOne({user_id:new mongodb.ObjectID(req.query.user_id),music_id:new mongodb.ObjectID(req.query.music_id),added_time:added_time}, function(err, results) {
        if(err){
            //resp.send(JSON.stringify({'status':'failed'}));
        }else{
            //resp.send(JSON.stringify({'status':'success'}));
    ...
    collection.find({user_id:new mongodb.ObjectID(req.query.user_id),music_id:new mongodb.ObjectID(req.query.music_id),added_time:added_time}).toArray(function(err, items) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','id':0}));
        } else {
            // resitem = items[0];
            ...
            if(items.length==0){
                ...
                collection.insert([{
                    ...
                    added_time: added_time,
                    //videoUrl: req.query.videoUrl,
                    music_id: new mongodb.ObjectID(req.query.music_id),
                    user_id:  new mongodb.ObjectID(req.query.user_id)
                }], function (err, result) {
                    if (err) {
                        resp.send(JSON.stringify({'status':'error','msg':'Database error occured. Try again!'}));
                        ...
                        //resp.send(JSON.stringify({'status': 'success'}));
                        collection.find({music_id:new mongodb.ObjectID(req.query.music_id)}).toArray(function(err, ival) {
                            resp.send(JSON.stringify({'status': 'success',countval:ival.length}));
                        });
                    }
                });
...
app.get('/addvideolike',function(req,resp){
    ...
    collection.insert([{
        ...
        if (err) {
            ...
            addhistorydata(req.query.videoid,req.query.user_id,'like',moment().unix());
            resp.send(JSON.stringify({'status': 'success'}));
        }
    });
...
app.get('/addvideolikegetupdate',function(req,resp){
    ...
    collection.insert([{
        ...
        if (err) {
            ...
            collection.find({videoid:new mongodb.ObjectID(req.query.videoid)}).toArray(function(err, ival) {
                addhistorydata(req.query.videoid,req.query.user_id,'like',moment().unix());
                resp.send(JSON.stringify({'status': 'success',countval:ival.length,item:ival}));
            });
        }
    });
...
app.get('/deletevideolikegetupdate',function (req,resp) {

    var added_time= moment().unix();
    var collection = db.collection('videolikes');
  /*  collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id), videoid:new mongodb.ObjectID(req.query.videoid) });*/
    /*collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id)});*/
    collection.deleteOne( { user_id: new mongodb.ObjectID(req.query.user_id) , videoid: new mongodb.ObjectID(req.query.videoid) }, function(err, results) {
        if(err){
            resp.send(JSON.stringify({'status':'failed'}));
        }else{
            //resp.send(JSON.stringify({'status':'success'}));
...
app.get('/deletenotification',function (req,resp) {

    var added_time= moment().unix();
    var collection = db.collection('notification');
    /*  collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id), videoid:new mongodb.ObjectID(req.query.videoid) });*/
    /*collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id)});*/
    collection.deleteOne( { post_id: new mongodb.ObjectID(req.query.post_id) , actionby: new mongodb.ObjectID(req.query.actionby) }, function(err, results) {
        if(err){
            resp.send(JSON.stringify({'status':'failed'}));
        }else{
            resp.send(JSON.stringify({'status':'success'}));
...
app.get('/deletecomment',function (req,resp) {
  ...
  /*  collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id), videoid:new mongodb.ObjectID(req.query.videoid) });*/
    /*collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id)});*/
    collection.deleteOne( { _id: new mongodb.ObjectID(req.query._id) , post_id: new mongodb.ObjectID(req.query.post_id) }, function(err, results) {
        if(err){
            resp.send(JSON.stringify({'status':'failed'}));
        }else{
            resp.send(JSON.stringify({'status':'success'}));
...
app.get('/deletecomment2',function (req,resp) {
  ...
  /*  collection.remove({ user_id:new mongodb.ObjectID(req.query.user_id), videoid:new mongodb.ObjectID(req.query.videoid) });*/
    ...
    collection.deleteOne( { _id: new mongodb.ObjectID(req.query._id) , post_id: new mongodb.ObjectID(req.query.post_id) }, function(err, results) {
        if(err){
            ...
            var collection1 = db.collection('commentview');
            //resp.send(JSON.stringify({'status':'success'}));
            collection1.find({post_id:new mongodb.ObjectID(req.query.post_id)}).toArray(function(err, ival) {
                resp.send(JSON.stringify({'status': 'success',countval:ival.length,item:ival}));
            });
        }
    } );
...
app.get('/deleteMultipleVideo',function(req,resp){
    var collection= db.collection('video');
    var x;

    for(x in req.query.data) {

        // collection.deleteOne({_id: new mongodb.ObjectID(x)}, true, true);

        collection.remove({_id: new mongodb.ObjectID(req.query.data[x])}, true);


    }
    resp.send(JSON.stringify({'status': 'success'}));
});
app.get('/deleteMultipleMusic',function(req,resp){
    var collection= db.collection('music');
    var x;
...
app.get('/addfavouritelist',function (req,resp) {

    var collection = db.collection('favouritelist');
    collection.insert([{

        favourite_id:  new mongodb.ObjectID(req.query.favourite_id),
        favourite_by_id:  new mongodb.ObjectID(req.query.favourite_by_id),
        added_time: moment().unix(),
    }], function (err, result) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','item':0}));
...
app.get('/blocklist',function (req,resp) {
...
app.get('/getfavouitelist',function (req,resp) {
    var collection= db.collection('favouritelist');
    collection.find({favourite_by_id : new mongodb.ObjectID(req.query.favourite_by_id)}).toArray(function(err, items) {
        if (err) {
            console.log(err);
            resp.send(JSON.stringify({'status':'error','item':[]}));
        } else {
...
app.get('/trendingArtistListAfterBlock',function (req,resp) {
    ...
    cond = {
        $and : [
            ...
            ]},{_id:{$nin:req.query.blockedidarray}}
        ]
    };
...
app.get('/userfanlistAfterBlock',function (req,resp) {
