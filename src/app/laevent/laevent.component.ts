...
@Component({
  selector: 'app-laevent',
  templateUrl: './laevent.component.html',
  styleUrls: ['./laevent.component.css'],
  providers: [Commonservices]
})
export class LaeventComponent implements OnInit {
  modalRef: BsModalRef;
  ...
  constructor(fb: FormBuilder,private _http: Http,private router: Router, private _commonservices : Commonservices,private route:ActivatedRoute, userdata: CookieService,private modalService: BsModalService,private sanitizer:DomSanitizer){
    this.chkerror = 0;
    ...
    var slideno = this.imagearr.length/5;
    if(slideno > Math.floor(slideno)){
      slideno = Math.floor(slideno)+1;
    }

    this.slideno = Array(slideno).fill(0).map((x,i)=>i);
  }
  ...
  ngOnInit() {
    this.dataForm = this.fb.group({
      ...
      email: ["", LaeventComponent.validateEmail],
      username: ["", LaeventComponent.validateUsername],
